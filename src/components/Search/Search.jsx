  ...
  return (
    ...
    <div className="flex items-center justify-center">
        ...
        <button
          ...
          className="bg-orange-500 hover:bg-blue-300 text-white font-bold py-1 px-1.5 border-orange-400 rounded-full relative right-10"
        >
          <SearchIcon/>
        </button>
        </div>
      ...
      <ul className="absolute top-[2.60rem] px-2  bg-slate-200 w-[20rem] ml-7 rounded-b-md border border-orange-300">
        {apiData.filter(filters=>filters.title.toLowerCase().includes(inputVal)).map((dat) => (
          <li className="border-b  border-slate-400 py-2" key={dat.id} >{dat.title}</li>
        ))}
      </ul>
    ):
    <div></div>}
